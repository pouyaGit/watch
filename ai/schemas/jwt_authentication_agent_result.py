"""JWT/authentication agent result schema (Stage R47.5).

A :class:`JWTAuthenticationAgentResultPlan` is the standard, R38-compatible
descriptive output of the JWT / authentication specialist research agent. It
...
- Research intelligence only: no HTTP/HTTPS request, no DNS resolution, no
  ...
  generation, no secret extraction, no execution logs, no persistence, no
  worker or scheduler.
...
- R37 integrated: ``governance_reference`` is a bounded reference to the R37
  governance export; unknown governance is never treated as authentication
  readiness.
...
- Observed evidence is preserved exactly as supplied; weakness or exploitation
  is never fabricated.
...
JWT_AUTHENTICATION_AGENT_RESULT_RULE_VERSION = "r47-5"
RULE_VERSION = JWT_AUTHENTICATION_AGENT_RESULT_RULE_VERSION

JWT_AUTHENTICATION_AGENT_STATUSES: tuple[str, ...] = AGENT_RESULT_STATUSES
...
JWT_AUTHENTICATION_AGENT_RESULT_LIMITATIONS: tuple[str, ...] = (
...
def sanitize_governance_reference(value: object) -> dict:
    ...
    valid governance or authentication readiness.
    """
...
def _bounded_identity(value: object) -> dict:
    return sanitize_jwt_authentication_agent_identity_plan(value)
...
def _bounded_hypotheses(value: object) -> list[dict]:
    ...
    for item in raw:
        if isinstance(item, dict):
            out.append(sanitize_jwt_authentication_hypothesis_plan(item))
        if len(out) >= MAX_HYPOTHESES:
            break
    return out


def sanitize_jwt_authentication_agent_result_plan(value: object) -> dict:
    """Project an R47.5 result onto its fixed bounded key set."""
    ...
    if not isinstance(value, dict):
        return {
            ...
            "status": "UNKNOWN",
            "context_analysis": (
                sanitize_jwt_authentication_context_analysis_plan(None)
            ),
            "hypotheses": [],
            "evidence_plan": (
                sanitize_jwt_authentication_evidence_plan(None)
            ),
            "confidence": "UNKNOWN",
            "limitations": [],
    ...
    if status not in JWT_AUTHENTICATION_AGENT_STATUSES:
        status = "UNKNOWN"
    ...
    return {
        ...
        "evidence_plan": sanitize_jwt_authentication_evidence_plan(
        ...
        "limitations": [
            ...
            if code in JWT_AUTHENTICATION_AGENT_RESULT_LIMITATIONS
        ][:MAX_LIMITATIONS],
...
class JWTAuthenticationAgentResultPlan(BaseModel):
    """Deterministic R38-compatible JWT/authentication result (R47.5)."""

    model_config = ConfigDict(extra="forbid")

    rule_version: str = JWT_AUTHENTICATION_AGENT_RESULT_RULE_VERSION
    agent_name: str = ""
    ...
    def _fixed_rule(cls, value: object) -> str:
        return JWT_AUTHENTICATION_AGENT_RESULT_RULE_VERSION
    ...
    def _valid_status(cls, value: object) -> str:
        text = _safe_text(value).strip().upper()
        if text not in JWT_AUTHENTICATION_AGENT_STATUSES:
    ...
    def _bounded_context(cls, value: object) -> dict:
        return sanitize_jwt_authentication_context_analysis_plan(value)
    ...
    def _bounded_evidence(cls, value: object) -> dict:
        return sanitize_jwt_authentication_evidence_plan(value)

    @field_validator("limitations")
    @classmethod
    def _valid_limitations(cls, value: list) -> list[str]:
        return _require_codes(
            value, JWT_AUTHENTICATION_AGENT_RESULT_LIMITATIONS,
            MAX_LIMITATIONS,
        )
    ...
    def _research_only(cls, value: object) -> bool:
        if not value:
            raise ValueError(
                "jwt/authentication agent results are research-only"
            )
        return True


def jwt_authentication_agent_result_plan_projection(
    value: JWTAuthenticationAgentResultPlan,
) -> dict:
    """Serialize an R47.5 result to a deterministic dict."""
...
__all__ = [
    ...
    "RULE_VERSION",
    "JWT_AUTHENTICATION_AGENT_STATUSES",
    "STATUS_CREATED",
    "STATUS_ANALYZING",
    "STATUS_COMPLETED",
    "STATUS_FAILED",
    ...
    "LIMITATION_GOVERNANCE_UNKNOWN",
    "LIMITATION_RESULT_UNKNOWN",
    "JWT_AUTHENTICATION_AGENT_RESULT_LIMITATIONS",
    "MAX_HYPOTHESES",
    "MAX_LIMITATIONS",
    "MAX_VALUE_LEN",
