"""SSRF agent result schema (Stage R40.5).

An :class:`SSRFAgentResultPlan` is the standard, R38-compatible descriptive
output of the SSRF specialist research agent. It answers the research
question:
...
- Research intelligence only: no network request, no DNS resolution, no
  ...
  execution logs, no persistence, no worker or scheduler.
...
from ai.schemas.ssrf_evidence_plan import sanitize_ssrf_evidence_plan
from ai.schemas.ssrf_hypothesis import sanitize_ssrf_hypothesis_plan

SSRF_AGENT_RESULT_RULE_VERSION = "r40-5"
RULE_VERSION = SSRF_AGENT_RESULT_RULE_VERSION

SSRF_AGENT_STATUSES: tuple[str, ...] = AGENT_RESULT_STATUSES
...
SSRF_AGENT_RESULT_LIMITATIONS: tuple[str, ...] = (
...
def _bounded_hypotheses(value: object) -> list[dict]:
    ...
    for item in raw:
        if isinstance(item, dict):
            out.append(sanitize_ssrf_hypothesis_plan(item))
...
def _bounded_identity(value: object) -> dict:
    return sanitize_ssrf_agent_identity_plan(value)


def sanitize_ssrf_agent_result_plan(value: object) -> dict:
    """Project an R40.5 result onto its fixed bounded key set."""
    ...
    if not isinstance(value, dict):
        return {
            ...
            "agent_identity": {},
            "status": "UNKNOWN",
            "context_analysis": sanitize_ssrf_context_analysis_plan(None),
            "hypotheses": [],
            "evidence_plan": sanitize_ssrf_evidence_plan(None),
            "confidence": "UNKNOWN",
            "limitations": [],
    ...
    if status not in SSRF_AGENT_STATUSES:
        status = "UNKNOWN"
    ...
    return {
        ...
        "status": status,
        "context_analysis": sanitize_ssrf_context_analysis_plan(
            value.get("context_analysis")
        ),
        "hypotheses": _bounded_hypotheses(value.get("hypotheses")),
        "evidence_plan": sanitize_ssrf_evidence_plan(
        ...
        "limitations": [
            ...
            if code in SSRF_AGENT_RESULT_LIMITATIONS
        ][:MAX_LIMITATIONS],
...
class SSRFAgentResultPlan(BaseModel):
    """Deterministic R38-compatible SSRF research agent result (R40.5)."""

    model_config = ConfigDict(extra="forbid")

    rule_version: str = SSRF_AGENT_RESULT_RULE_VERSION
    agent_name: str = ""
    ...
    def _fixed_rule(cls, value: object) -> str:
        return SSRF_AGENT_RESULT_RULE_VERSION
    ...
    def _valid_status(cls, value: object) -> str:
        text = _safe_text(value).strip().upper()
        if text not in SSRF_AGENT_STATUSES:
    ...
    def _bounded_context(cls, value: object) -> dict:
        return sanitize_ssrf_context_analysis_plan(value)
    ...
    def _bounded_evidence(cls, value: object) -> dict:
        return sanitize_ssrf_evidence_plan(value)

    @field_validator("limitations")
    @classmethod
    def _valid_limitations(cls, value: list) -> list[str]:
        return _require_codes(
            value, SSRF_AGENT_RESULT_LIMITATIONS, MAX_LIMITATIONS
        )
    ...
    def _research_only(cls, value: object) -> bool:
        if not value:
            raise ValueError("ssrf agent results are research-only")
        return True


def ssrf_agent_result_plan_projection(value: SSRFAgentResultPlan) -> dict:
    """Serialize an SSRF agent result to a deterministic dict."""
