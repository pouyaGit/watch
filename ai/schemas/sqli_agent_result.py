"""SQLi agent result schema (Stage R41.5).

An :class:`SQLIAgentResultPlan` is the standard, R38-compatible descriptive
output of the SQL injection specialist research agent. It answers the
research question:
...
- Research intelligence only: no SQL execution, no database connection, no
  SQL payload storage or generation, no execution logs, no HTTP/network
  request, no fuzzing, no persistence, no worker or scheduler.
...
- Provenance: ``provenance`` records which R31-R37 input layers were
  supplied through the R38 read-only input contract.
...
from ai.schemas.sqli_evidence_plan import sanitize_sqli_evidence_plan
from ai.schemas.sqli_hypothesis import sanitize_sqli_hypothesis_plan

SQLI_AGENT_RESULT_RULE_VERSION = "r41-5"
RULE_VERSION = SQLI_AGENT_RESULT_RULE_VERSION

SQLI_AGENT_STATUSES: tuple[str, ...] = AGENT_RESULT_STATUSES
...
SQLI_AGENT_RESULT_LIMITATIONS: tuple[str, ...] = (
...
def sanitize_governance_reference(value: object) -> dict:
    ...
    version and all four component records; anything else (missing,
    malformed, foreign version, component-less) degrades to ``UNKNOWN`` with
    ``ready=False`` and is never treated as valid governance.
    """
...
def _bounded_identity(value: object) -> dict:
    return sanitize_sqli_agent_identity_plan(value)
...
def _bounded_hypotheses(value: object) -> list[dict]:
    ...
    for item in raw:
        if isinstance(item, dict):
            out.append(sanitize_sqli_hypothesis_plan(item))
        if len(out) >= MAX_HYPOTHESES:
            break
    return out


def sanitize_sqli_agent_result_plan(value: object) -> dict:
    """Project an R41.5 result onto its fixed bounded key set."""
    ...
    if not isinstance(value, dict):
        return {
            ...
            "agent_identity": {},
            "status": "UNKNOWN",
            "context_analysis": sanitize_sqli_context_analysis_plan(None),
            "hypotheses": [],
            "evidence_plan": sanitize_sqli_evidence_plan(None),
            "confidence": "UNKNOWN",
            "limitations": [],
    ...
    if status not in SQLI_AGENT_STATUSES:
        status = "UNKNOWN"
    ...
    return {
        ...
        "status": status,
        "context_analysis": sanitize_sqli_context_analysis_plan(
            value.get("context_analysis")
        ),
        "hypotheses": _bounded_hypotheses(value.get("hypotheses")),
        "evidence_plan": sanitize_sqli_evidence_plan(
        ...
        "limitations": [
            ...
            if code in SQLI_AGENT_RESULT_LIMITATIONS
        ][:MAX_LIMITATIONS],
...
class SQLIAgentResultPlan(BaseModel):
    """Deterministic R38-compatible SQLi research agent result (R41.5)."""

    model_config = ConfigDict(extra="forbid")

    rule_version: str = SQLI_AGENT_RESULT_RULE_VERSION
    agent_name: str = ""
    ...
    def _fixed_rule(cls, value: object) -> str:
        return SQLI_AGENT_RESULT_RULE_VERSION
    ...
    def _valid_status(cls, value: object) -> str:
        text = _safe_text(value).strip().upper()
        if text not in SQLI_AGENT_STATUSES:
    ...
    def _bounded_context(cls, value: object) -> dict:
        return sanitize_sqli_context_analysis_plan(value)
    ...
    def _bounded_evidence(cls, value: object) -> dict:
        return sanitize_sqli_evidence_plan(value)

    @field_validator("limitations")
    @classmethod
    def _valid_limitations(cls, value: list) -> list[str]:
        return _require_codes(
            value, SQLI_AGENT_RESULT_LIMITATIONS, MAX_LIMITATIONS
        )
    ...
    def _research_only(cls, value: object) -> bool:
        if not value:
            raise ValueError("sqli agent results are research-only")
        return True


def sqli_agent_result_plan_projection(value: SQLIAgentResultPlan) -> dict:
    """Serialize a SQLi agent result to a deterministic dict."""
