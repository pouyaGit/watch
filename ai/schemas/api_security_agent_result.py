"""API security agent result schema (Stage R49.5).

An :class:`APISecurityAgentResultPlan` is the standard, R38-compatible
descriptive output of the API security specialist research agent. It
...
- R37 integrated: ``governance_reference`` is a bounded reference to the
  R37 governance export; unknown governance is never treated as API
  security readiness.
...
API_SECURITY_AGENT_RESULT_RULE_VERSION = "r49-5"
RULE_VERSION = API_SECURITY_AGENT_RESULT_RULE_VERSION

API_SECURITY_AGENT_STATUSES: tuple[str, ...] = AGENT_RESULT_STATUSES
...
API_SECURITY_AGENT_RESULT_LIMITATIONS: tuple[str, ...] = (
...
def sanitize_governance_reference(value: object) -> dict:
    ...
    valid governance or API security readiness.
    """
...
def _bounded_identity(value: object) -> dict:
    return sanitize_api_security_agent_identity_plan(value)
...
def _bounded_hypotheses(value: object) -> list[dict]:
    ...
    for item in raw:
        if isinstance(item, dict):
            out.append(sanitize_api_security_hypothesis_plan(item))
        if len(out) >= MAX_HYPOTHESES:
            break
    return out


def sanitize_api_security_agent_result_plan(value: object) -> dict:
    """Project an R49.5 result onto its fixed bounded key set."""
    ...
    if not isinstance(value, dict):
        return {
            ...
            "status": "UNKNOWN",
            "context_analysis": (
                sanitize_api_security_context_analysis_plan(None)
            ),
            "hypotheses": [],
            "evidence_plan": sanitize_api_security_evidence_plan(None),
            "confidence": "UNKNOWN",
            "limitations": [],
    ...
    if status not in API_SECURITY_AGENT_STATUSES:
        status = "UNKNOWN"
    ...
    return {
        ...
        "status": status,
        "context_analysis": sanitize_api_security_context_analysis_plan(
            value.get("context_analysis")
        ),
        "hypotheses": _bounded_hypotheses(value.get("hypotheses")),
        "evidence_plan": sanitize_api_security_evidence_plan(
        ...
        "limitations": [
            ...
            if code in API_SECURITY_AGENT_RESULT_LIMITATIONS
        ][:MAX_LIMITATIONS],
...
class APISecurityAgentResultPlan(BaseModel):
    """Deterministic R38-compatible API security result (R49.5)."""

    model_config = ConfigDict(extra="forbid")

    rule_version: str = API_SECURITY_AGENT_RESULT_RULE_VERSION
    agent_name: str = ""
    ...
    def _fixed_rule(cls, value: object) -> str:
        return API_SECURITY_AGENT_RESULT_RULE_VERSION
    ...
    def _valid_status(cls, value: object) -> str:
        text = _safe_text(value).strip().upper()
        if text not in API_SECURITY_AGENT_STATUSES:
    ...
    def _bounded_context(cls, value: object) -> dict:
        return sanitize_api_security_context_analysis_plan(value)
    ...
    def _bounded_evidence(cls, value: object) -> dict:
        return sanitize_api_security_evidence_plan(value)

    @field_validator("limitations")
    @classmethod
    def _valid_limitations(cls, value: list) -> list[str]:
        return _require_codes(
            value, API_SECURITY_AGENT_RESULT_LIMITATIONS, MAX_LIMITATIONS
        )
    ...
    def _research_only(cls, value: object) -> bool:
        if not value:
            raise ValueError("api security agent results are research-only")
        return True


def api_security_agent_result_plan_projection(
    value: APISecurityAgentResultPlan,
) -> dict:
    """Serialize an R49.5 result to a deterministic dict."""
...
__all__ = [
    ...
    "LIMITATION_GOVERNANCE_UNKNOWN",
    "LIMITATION_RESULT_UNKNOWN",
    "API_SECURITY_AGENT_RESULT_LIMITATIONS",
    "MAX_HYPOTHESES",
    "MAX_LIMITATIONS",
    "MAX_VALUE_LEN",
