"""CVE research agent result schema (Stage R50.5).

A :class:`CVEResearchAgentResultPlan` is the standard, R38-compatible
descriptive output of the CVE research specialist agent. It answers the
research question:
...
- Research intelligence only: no NVD/vendor API call, no web request, no
  ...
  socket, no DNS, no database, no scanner, no vulnerability
  reproduction, no payload storage or generation, no secret extraction,
  no execution, no persistence, no worker or scheduler.
...
- R37 integrated: ``governance_reference`` is a bounded reference to the
  R37 governance export; unknown governance is never treated as CVE
  research readiness.
- Provenance: ``provenance`` records which R31-R37 input layers were
  supplied through the R38 read-only input contract; no layer is ever
  invented.
...
CVE_RESEARCH_AGENT_RESULT_RULE_VERSION = "r50-5"
RULE_VERSION = CVE_RESEARCH_AGENT_RESULT_RULE_VERSION

CVE_RESEARCH_AGENT_STATUSES: tuple[str, ...] = AGENT_RESULT_STATUSES
...
LIMITATION_NO_SECRET_EXTRACTION = "NO_SECRET_EXTRACTION"
...
CVE_RESEARCH_AGENT_RESULT_LIMITATIONS: tuple[str, ...] = (
...
def sanitize_governance_reference(value: object) -> dict:
    ...
    valid governance or CVE research readiness.
    """
...
def _bounded_identity(value: object) -> dict:
    return sanitize_cve_research_agent_identity_plan(value)
...
def _bounded_hypotheses(value: object) -> list[dict]:
    ...
    for item in raw:
        if isinstance(item, dict):
            out.append(sanitize_cve_research_hypothesis_plan(item))
        if len(out) >= MAX_HYPOTHESES:
            break
    return out


def sanitize_cve_research_agent_result_plan(value: object) -> dict:
    """Project an R50.5 result onto its fixed bounded key set."""
    ...
    if not isinstance(value, dict):
        return {
            ...
            "status": "UNKNOWN",
            "context_analysis": (
                sanitize_cve_research_context_analysis_plan(None)
            ),
            "hypotheses": [],
            "evidence_plan": sanitize_cve_research_evidence_plan(None),
            "confidence": "UNKNOWN",
            "limitations": [],
    ...
    if status not in CVE_RESEARCH_AGENT_STATUSES:
        status = "UNKNOWN"
    ...
    return {
        ...
        "status": status,
        "context_analysis": sanitize_cve_research_context_analysis_plan(
            value.get("context_analysis")
        ),
        "hypotheses": _bounded_hypotheses(value.get("hypotheses")),
        "evidence_plan": sanitize_cve_research_evidence_plan(
        ...
        "limitations": [
            ...
            if code in CVE_RESEARCH_AGENT_RESULT_LIMITATIONS
        ][:MAX_LIMITATIONS],
...
class CVEResearchAgentResultPlan(BaseModel):
    """Deterministic R38-compatible CVE research result (R50.5)."""

    model_config = ConfigDict(extra="forbid")

    rule_version: str = CVE_RESEARCH_AGENT_RESULT_RULE_VERSION
    agent_name: str = ""
    ...
    def _fixed_rule(cls, value: object) -> str:
        return CVE_RESEARCH_AGENT_RESULT_RULE_VERSION
    ...
    def _valid_status(cls, value: object) -> str:
        text = _safe_text(value).strip().upper()
        if text not in CVE_RESEARCH_AGENT_STATUSES:
    ...
    def _bounded_context(cls, value: object) -> dict:
        return sanitize_cve_research_context_analysis_plan(value)
    ...
    def _bounded_evidence(cls, value: object) -> dict:
        return sanitize_cve_research_evidence_plan(value)

    @field_validator("limitations")
    @classmethod
    def _valid_limitations(cls, value: list) -> list[str]:
        return _require_codes(
            value, CVE_RESEARCH_AGENT_RESULT_LIMITATIONS, MAX_LIMITATIONS
        )
    ...
    def _research_only(cls, value: object) -> bool:
        if not value:
            raise ValueError("cve research agent results are research-only")
        return True


def cve_research_agent_result_plan_projection(
    value: CVEResearchAgentResultPlan,
) -> dict:
    """Serialize an R50.5 result to a deterministic dict."""
...
__all__ = [
    ...
    "LIMITATION_GOVERNANCE_UNKNOWN",
    "LIMITATION_RESULT_UNKNOWN",
    "CVE_RESEARCH_AGENT_RESULT_LIMITATIONS",
    "MAX_HYPOTHESES",
    "MAX_LIMITATIONS",
    "MAX_VALUE_LEN",
