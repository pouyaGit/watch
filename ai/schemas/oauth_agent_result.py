"""OAuth agent result schema (Stage R48.5).

An :class:`OAuthAgentResultPlan` is the standard, R38-compatible
descriptive output of the OAuth specialist research agent. It answers the
research question:
...
- Research intelligence only: no HTTP/HTTPS request, no OAuth
  ...
  execution, no persistence, no worker or scheduler.
...
- R37 integrated: ``governance_reference`` is a bounded reference to the
  R37 governance export; unknown governance is never treated as OAuth
  readiness.
- Provenance: ``provenance`` records which R31-R37 input layers were
  ...
  invented.
- Observed evidence is preserved exactly as supplied; weakness or
  exploitation is never fabricated.
...
from ai.schemas.oauth_evidence_plan import sanitize_oauth_evidence_plan
from ai.schemas.oauth_hypothesis import sanitize_oauth_hypothesis_plan
...
OAUTH_AGENT_RESULT_RULE_VERSION = "r48-5"
RULE_VERSION = OAUTH_AGENT_RESULT_RULE_VERSION

OAUTH_AGENT_STATUSES: tuple[str, ...] = AGENT_RESULT_STATUSES
...
OAUTH_AGENT_RESULT_LIMITATIONS: tuple[str, ...] = (
...
def sanitize_governance_reference(value: object) -> dict:
    ...
    valid governance or OAuth readiness.
    """
...
def _bounded_identity(value: object) -> dict:
    return sanitize_oauth_agent_identity_plan(value)
...
def _bounded_hypotheses(value: object) -> list[dict]:
    ...
    for item in raw:
        if isinstance(item, dict):
            out.append(sanitize_oauth_hypothesis_plan(item))
        if len(out) >= MAX_HYPOTHESES:
            break
    return out


def sanitize_oauth_agent_result_plan(value: object) -> dict:
    """Project an R48.5 result onto its fixed bounded key set."""
    ...
    if not isinstance(value, dict):
        return {
            ...
            "status": "UNKNOWN",
            "context_analysis": (
                sanitize_oauth_context_analysis_plan(None)
            ),
            "hypotheses": [],
            "evidence_plan": sanitize_oauth_evidence_plan(None),
            "confidence": "UNKNOWN",
            "limitations": [],
    ...
    if status not in OAUTH_AGENT_STATUSES:
        status = "UNKNOWN"
    ...
    return {
        ...
        "status": status,
        "context_analysis": sanitize_oauth_context_analysis_plan(
            value.get("context_analysis")
        ),
        "hypotheses": _bounded_hypotheses(value.get("hypotheses")),
        "evidence_plan": sanitize_oauth_evidence_plan(
        ...
        "limitations": [
            ...
            if code in OAUTH_AGENT_RESULT_LIMITATIONS
        ][:MAX_LIMITATIONS],
...
class OAuthAgentResultPlan(BaseModel):
    """Deterministic R38-compatible OAuth result (R48.5)."""

    model_config = ConfigDict(extra="forbid")

    rule_version: str = OAUTH_AGENT_RESULT_RULE_VERSION
    agent_name: str = ""
    ...
    def _fixed_rule(cls, value: object) -> str:
        return OAUTH_AGENT_RESULT_RULE_VERSION
    ...
    def _valid_status(cls, value: object) -> str:
        text = _safe_text(value).strip().upper()
        if text not in OAUTH_AGENT_STATUSES:
    ...
    def _bounded_context(cls, value: object) -> dict:
        return sanitize_oauth_context_analysis_plan(value)
    ...
    def _bounded_evidence(cls, value: object) -> dict:
        return sanitize_oauth_evidence_plan(value)

    @field_validator("limitations")
    @classmethod
    def _valid_limitations(cls, value: list) -> list[str]:
        return _require_codes(
            value, OAUTH_AGENT_RESULT_LIMITATIONS, MAX_LIMITATIONS
        )
    ...
    def _research_only(cls, value: object) -> bool:
        if not value:
            raise ValueError("oauth agent results are research-only")
        return True


def oauth_agent_result_plan_projection(value: OAuthAgentResultPlan) -> dict:
    """Serialize an R48.5 result to a deterministic dict."""
...
__all__ = [
    ...
    "LIMITATION_GOVERNANCE_UNKNOWN",
    "LIMITATION_RESULT_UNKNOWN",
    "OAUTH_AGENT_RESULT_LIMITATIONS",
    "MAX_HYPOTHESES",
    "MAX_LIMITATIONS",
    "MAX_VALUE_LEN",
