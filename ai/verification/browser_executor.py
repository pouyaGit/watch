"""Legacy World A browser evidence executor — NON-PRODUCTION (Phase P1).

Unconditional browser-automation executor (playwright, no
authorization/scope gates, no 5B→5J binding). NOT part of the frozen
5B→5J authority chain and MUST NOT be reached from any production
path: the sole production caller was watch_xss_verify (entrypoint
permanently disabled in Phase 5K) and the task registry contains no
entry that can reach it. Importable for offline unit tests only.
There is no flag or environment variable that re-enables legacy
