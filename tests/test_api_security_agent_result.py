"""tests/test_api_security_agent_result.py — Stage R49.5 tests.

Deterministic, offline tests for the API security agent result:
...
from ai.schemas import api_security_agent_result as schema
...
def needs_evidence_result():
    return export_api_security_agent_result(api_type="REST")
...
class TestAPISecurityAgentResult(unittest.TestCase):
    def test_key_set_is_exact(self):
        result = export_api_security_agent_result()
        self.assertEqual(
            set(result.keys()),
        ...
        self.assertEqual(result["rule_version"], "r49-5")
    ...
    def test_identity_embedded(self):
        ...
        self.assertEqual(result["agent_name"], "api-security-specialist")

    def test_status_derivation(self):
        self.assertEqual(
            export_api_security_agent_result()["status"], "CREATED"
        )
        self.assertEqual(
    ...
    def test_confidence_calibration_through_result(self):
        self.assertEqual(
            export_api_security_agent_result()["confidence"], "UNKNOWN"
        )
    ...
    def test_observed_weakness_preserved(self):
        ...
        authentication = [
            ...
            if hypothesis["hypothesis_type"] == "API_AUTHENTICATION_GAP"
        ][0]
        self.assertEqual(
            authentication["hypothesis_state"], "WEAKNESS_OBSERVED"
        )
        self.assertEqual(authentication["priority"], "HIGH")
        self.assertIn(
            "API_AUTHENTICATION_ABSENT_OBSERVED",
            authentication["supporting_signals"],
        )
        self.assertEqual(
            result["context_analysis"]["api_authentication"],
            "ABSENT_OBSERVED",
        )
    ...
    def test_no_fabricated_weakness(self):
        ...
        self.assertNotIn("API_BYPASS_OBSERVED", serialized)
        self.assertNotIn("api bypassed", serialized.lower())

    def test_provenance_unknown_by_default(self):
        provenance = export_api_security_agent_result()["provenance"]
    ...
    def test_r42_compatibility(self):
        ...
        self.assertEqual(evaluation["evaluated_agent_category"], "RECON")
        self.assertEqual(
            evaluation["evaluated_result_rule_version"], "r49-5"
        )
    ...
    def test_observed_weakness_is_research_finding_only(self):
        ...
        for gap in gaps:
            ...
            self.assertIn("NO_API_ATTACK_CLAIM", gap["limitations"])
    ...
    def test_r43_collaboration_with_r39(self):
        ...
        self.assertEqual(categories, {"XSS", "RECON"})
    ...
    def test_r43_collaboration_with_r40(self):
        ...
        self.assertEqual(categories, {"SSRF", "RECON"})
        self.assertEqual(len(collaboration["collaboration_rankings"]), 2)
    ...
    def test_r43_collaboration_with_r41(self):
        ...
        self.assertEqual(categories, {"SQLI", "RECON"})
        self.assertEqual(len(collaboration["collaboration_rankings"]), 2)
    ...
    def test_r43_collaboration_with_r46(self):
        ...
        self.assertEqual(categories, {"IDOR", "RECON"})
    ...
    def test_r43_collaboration_with_r47(self):
        ...
        self.assertEqual(categories, {"JWT", "RECON"})
        first = json.dumps(collaboration, sort_keys=True)
    ...
    def test_r43_collaboration_with_r48(self):
        ...
        self.assertEqual(categories, {"OAUTH", "RECON"})
        first = json.dumps(collaboration, sort_keys=True)
    ...
    def test_r44_feedback_compatibility(self):
        ...
        self.assertEqual(event["source_category"], "RECON")
        classifications = classify_research_feedback_events([event])
        self.assertEqual(len(classifications), 1)
        self.assertEqual(classifications[0]["subject"], "RECON")
    ...
    def test_forbidden_payload_secret_or_attack_output(self):
        ...
        self.assertNotIn("access_token_value", serialized)
    ...
    def test_r38_result_projection(self):
        projection = api_security_agent_result_to_r38(weakness_result())
    ...
    def test_result_model_rejects_bad_values_and_extra(self):
        ...
        with self.assertRaises(ValidationError):
            schema.APISecurityAgentResultPlan(
                **{**base, "status": "NOPE"}
            )
        with self.assertRaises(ValidationError):
            schema.APISecurityAgentResultPlan(
        ...
        with self.assertRaises(ValidationError):
            schema.APISecurityAgentResultPlan(**{**base, "token": "x"})

    def test_result_model_forces_rule_version(self):
        plan = schema.APISecurityAgentResultPlan(rule_version="r99-9")
        self.assertEqual(plan.rule_version, "r49-5")
