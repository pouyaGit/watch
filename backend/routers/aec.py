"""Read-only AEC views for the command center (EPIC 3 Part 4).

Three GET endpoints over explicitly supplied view data — case records,
queue snapshot, aggregate status. Handlers are pure allowlist
projections: they select a fixed field set and drop everything else, so
unmounted or mis-shaped input can neither leak nor mutate. There is no
database, no session, and no write path anywhere in this module.
